...
@section('content')
    <title>إدارة رحلات السيارات</title>

    {{--    <link rel="stylesheet" href="{{ asset('css/range_price.css') }}">--}}
    <div class="container-xl">
        <div class="card shadow-lg bg-white py-4 px-4" style="border-radius: 40px">
            ...
            <h3 class="text-primary text-center">إدارة رحلات السيارات</h3>
            <br>
                            ...
                            <div class="row">
                                ...
                                <div class="col-9">
                                    <select class="form-control" name="car">
                                        ...
                                        @foreach($cars as $car)
                                            <option value="{{ $car->id }}" {{ ($car_get == 'all') ? "selected":"" }}>{{ "($car->id). $car->type" }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
            ...
            <table class="table table-bordered table-secondary table-responsive-lg">
                ...
                @foreach($orders as $order)
                    <tr>
                        ...
                        <td>
                            @if(isset($order->car))
                                <a href="{{ route('admin.panel.car.edit', $order->car_id) }}">{{ $order->car->type ?? "غير محدد" }}</a>
                            @else
                                غير محدد
                            @endif
                        </td>
                        ...
                        <td>{{ $order->distance_kilo ?? 0 }} كيلو</td>
                        <td>
                            @if($order->status == 0)
                        ...
                        <td>
                            <a href="{{ route('admin.panel.car.delete', $order->car_id) }}" class="text-decoration-none btn btn-danger">
                                حذف_السيارة
                            </a>
                        </td>
                    </tr>
